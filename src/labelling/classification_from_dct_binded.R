...
load(file.path(ruta_modelos, "binded_50_1.RData"))
modelo_binded <- lista_modelos[[4]]
########################################################
...
crop_binded <- 15
crop_puni <- crop_features(pix_puni_dct, length(clases_puni), crop_binded)
crop_pright <- crop_features(pix_pright_dct, length(clases_pright), crop_binded)
crop_pleft <- crop_features(pix_pleft_dct, length(clases_pleft), crop_binded)
########################################################
...
lab_puni <- asignar_etiquetas(crop_puni, length(crop_puni), modelo_binded)
lab_pright <- asignar_etiquetas(crop_pright, length(crop_pright), modelo_binded)
lab_pleft <- asignar_etiquetas(crop_pleft, length(crop_pleft), modelo_binded)
########################################################
...
########################################################
# Guardar resultados
save(img_lab_puni, img_lab_pright, img_lab_pleft, file = file.path(ruta,"class_maps_binded_1.RData"))
########################################################
